...
Use this guide for running, backing up, and recovering a single-server
installation. [Configuration](configuration.md) lists settings;
[persistence](persistence.md) identifies authoritative data.

## Production deployment

Use the [release and deployment](deployment.md) transaction for the supported
Debian guest. The deployment runner verifies release artifacts; the local Rust
manager owns configuration, backup, admission, and rollback.

Run a dedicated unprivileged account with managed-root read/write/create/rename/
sync access, read access to configured import roots, configuration, and injected
secrets, and no access to unrelated source trees. Require a filesystem with
ordinary permissions and same-filesystem atomic rename. Import also probes Linux
beneath-open, file/directory sync, and atomic no-replace publication. Run one
server per data root, including on shared filesystems.

Serve the complete Trunk distribution separately behind TLS. Enable secure
cookies, disable local admin login, restrict exact browser origins, and keep
callback hostnames consistent. The public `labello.client.json` may be replaced
without rebuilding WASM; it is fetched with no-store after reload. A missing file
must return 404 rather than `index.html`. See [configuration](configuration.md).

Restrict log access and retention: even safe IDs and aggregate activity reveal
...
not emit a dedicated lifecycle event. There are no dedicated
`import.failed` or `import.cancelled` events.
...
operational cleanup guarantee. Terminal job metadata and API control records can continue to grow.
...
availability externally, without assuming richer import metrics are available.
...
Requests wait in separate bounded thumbnail and foreground queues; a full queue
or unavailable cache produces an error. With at least two workers, thumbnails
leave one slot available for foreground work. See [preview limits](configuration.md#image-preview-limits).
Working-image loads use Data Saver and never automatically fetch a larger representation.
...
The login page discovers methods and checks the session separately; each failed
request has its own retry. A current API 401 coalesces a session recheck. Valid
sessions retain role-denial errors; expired sessions retain drafts for their
original account. Changing endpoint clears account/dataset/import ownership.
See [getting started](getting-started.md#connect-and-sign-in) and
[draft recovery](annotation.md#drafts-feedback-and-connection-state).

Raw browser upload failures also request session revalidation because their
adapters lack structured HTTP status. Local folder-selection errors do not.
...
Lifecycle and HTTP completion use INFO; workflow operations and expected signed-out
requests use DEBUG. Authorization/resource limits and recoverable data/cache
failures use WARN. Dependency/internal errors, corrupt authentication, poisoned
locks, and failed background jobs use ERROR. Request failure categorization follows
the table below.
