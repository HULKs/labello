...
impl StatsAggregation {
    pub(super) fn record_contributors(&mut self, state: &ImageState, events: &[EventLogEntry]) {
        ...
        for event in events {
            let review = match &event.payload {
                ...
                | EventPayload::ReviewerCorrectionRecorded { review, .. } => review,
                _ => continue,
            };
            ...
            self.contributors
                .day(&review.reviewer_user_id, review.timestamp)
                .reviewed += 1;
            ...
            if let Some(author) = author {
                let day = self.contributors.day(author, review.timestamp);
