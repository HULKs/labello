...
mod tests {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    #[tokio::test]
    async fn server_presence_preserves_api_prefix_and_decodes_users_and_datasets() {
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let address = listener.local_addr().unwrap();
        let server = std::thread::spawn(move || {
            let (mut stream, _) = listener.accept().unwrap();
            ...
            write_json_response(
                &mut stream,
                r#"{"users":[{"userId":"alice","datasets":[{"datasetId":"ds","name":"Robots"}]}]}"#,
            );
            request
        });
        let api = HttpLabelloApi::new(format!("http://{address}/api/")).unwrap();
        let value = api.server_presence().await.unwrap();
        assert_eq!(value.users[0].user_id, UserId::from("alice"));
        assert_eq!(value.users[0].datasets[0].name, "Robots");
        let (headers, body) = server.join().unwrap();
        assert!(headers.starts_with("GET /api/presence HTTP/1.1"));
