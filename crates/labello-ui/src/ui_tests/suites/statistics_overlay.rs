...
fn leaderboard_streak_sort_and_inline_name_layout() {
    ...
    for width in [320.0, 390.0, 600.0] {
        ...
        harness.run_steps(4);
        let order = harness.get_by_label("Sort rankings").rect();
        assert!(order.height() >= 44.0 && order.right() <= width);
        harness.get_by_label("Sort rankings").focus();
        harness.key_press(egui::Key::Space);
        harness.run_steps(3);
        harness.get_by_role_and_label(egui::accesskit::Role::Button, "Streak").focus();
        harness.key_press(egui::Key::Space);
        harness.run_steps(3);
        assert_eq!(harness.get_by_label("Sort rankings").value().as_deref(), Some("Streak"));
        if harness.query_by_role_and_label(egui::accesskit::Role::Button, "Streak").is_some() {
            harness.key_press(egui::Key::Escape);
            harness.run_steps(3);
        }
        assert!(harness.state().navigation.statistics.open);
        harness.get_by_label("Reverse ranking order").click();
        harness.run_steps(3);
