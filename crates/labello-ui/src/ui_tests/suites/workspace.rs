...
fn review_correction_drawer_and_actions_stay_reachable() {
    ...
    for (width, height) in viewport_sizes() {
        ...
        let finalize =
            harness.get_by_role_and_label(egui::accesskit::Role::Button, "Keep correction");
    ...
    for (width, height) in [(320.0, 320.0), (600.0, 568.0), (600.0, 320.0)] {
        ...
        harness.step();
        harness
            .get_by_role_and_label(egui::accesskit::Role::Button, "Keep correction")
            .scroll_to_me();
        for _ in 0..8 {
            harness.step();
        }
...
fn work_workflow_draws_saves_submits_and_reviews() {
    ...
    assert!(harness.query_by_label("Submit & reject").is_none());
    ...
    assert!(harness.query_by_label("Submit & reject").is_some());
    assert!(harness.query_by_label("Accept").is_none());
    harness.key_press(egui::Key::Y);
    harness.step();
    step_until(&mut harness, 10, |app| !app.loading.saving);
    ...
    click(&mut harness, "Submit & reject");
    step_until(&mut harness, 10, |app| !app.loading.saving);
...
fn reviewer_correction_controls_follow_task_config_and_keep_an_isolated_bbox_draft() {
    ...
    assert!(disabled.query_by_label("Correct object").is_some());
    ...
    harness.get_by_role_and_label(egui::accesskit::Role::Button, "Keep correction").scroll_to_me();
    harness.run_steps(8);
    assert_control_inside(&harness, "Keep correction", egui::accesskit::Role::Button, 1500.0, 780.0);
    click(&mut harness, "Keep correction");
...
fn review_revision_stages_decisions_preserves_cancelled_drafts_and_retries_identical_commit() {
    ...
    assert!(harness.state().can_correct_review_object());
    assert!(harness.query_by_label("Correct object").is_some());
...
#[test]
fn delayed_previous_review_keeps_canvas_busy_state_and_replaces_on_success() {
    let api = Rc::new(SpyApi::new());
    seed_review_annotation(
        &api,
        AnnotationGeometry::BoundingBox(BoundingBox {
    ...
    step_until(&mut harness, 16, |app| {
        app.work
            .assignment
            .as_ref()
            .is_some_and(|assignment| assignment.image_id != first_image)
            && app.work.previous_assignment.is_some()
            && !app.loading.saving
    });
    let current_image = harness
        ...
        .image_id
        .clone();
    let current_texture = harness.state().work.current_texture.as_ref().unwrap().id();
    ...
    let canvas_still_painted = harness
        .output()
        .shapes
        .iter()
        .any(|clipped| has_texture(&clipped.shape, current_texture));
    assert!(
        canvas_still_painted,
        "the current image texture must remain painted while Previous is pending"
    );
    ...
    harness.state_mut().runtime.native_task_spawner = None;
    harness.step();
    step_until(&mut harness, 20, |app| {
        app.work
            .assignment
...
#[test]
fn delayed_confirmed_previous_review_hides_modal_and_keeps_canvas() {
    let api = Rc::new(SpyApi::new());
    seed_review_annotation(
        &api,
        AnnotationGeometry::BoundingBox(BoundingBox {
    ...
    api.state
        .borrow_mut()
        .states
        .insert(second_image, second_state);
    step_until(&mut harness, 12, |app| app.work.queue.len() == 2);
    ...
    step_until(&mut harness, 16, |app| {
        app.work
            .assignment
            .as_ref()
            .is_some_and(|assignment| assignment.image_id == ImageId::from("img_2"))
            && app.work.previous_assignment.is_some()
    ...
    assert!(
        harness
            .query_by_role_and_label(egui::accesskit::Role::Button, "Keep correction")
...
#[test]
fn delayed_previous_review_failure_preserves_current_canvas() {
    let api = Rc::new(SpyApi::new());
    seed_review_annotation(
        &api,
        AnnotationGeometry::BoundingBox(BoundingBox {
    ...
    step_until(&mut harness, 12, |app| {
        !app.loading.image
            && app
                ...
                .error
                .as_deref()
                .is_some_and(|error| error.contains("assignment cannot be reopened"))
    });
        ...
        match shape {
            egui::Shape::Circle(circle)
                if circle.radius == 4.0
                    && circle.fill != egui::Color32::TRANSPARENT
...
fn reviewer_can_stage_multiple_additions_and_distinguish_unsaved_previews() {
    ...
    let persisted = harness.state().work.annotations.clone();
    for _ in 0..2 {
        click(&mut harness, "Add missing annotation");
        harness.step();
        click(&mut harness, "Keep correction");
        harness.step();
    }
