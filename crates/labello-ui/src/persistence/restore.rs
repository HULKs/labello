impl crate::app::LabelloApp {
    ...
    pub(crate) fn reopen_previous_workspace(&mut self) {
        ...
        if !authorized {
            self.runtime.notice = Some("The previous view is no longer authorized; choose an available dataset view.".to_string());
            return;
        }
            ...
            DraftRecovery::Work(draft, DraftValidation::Valid) => {
                match draft.payload {
                    ...
                    WorkDraftPayload::Review(draft) => {
                        ...
                        self.work.assignment_touched |= draft.correction.is_some() || !draft.staged_corrections.changes.is_empty();
