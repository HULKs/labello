...
Implement the issue against the real product path and leave enough evidence to challenge every completion claim. Treat implementation and independent acceptance as separate jobs.
...
3. Use the pull-request author as the accountable human owner. Before a pull request exists, the planned owner is the authenticated GitHub user who will author it. Once it exists, read the author from GitHub and treat that identity as authoritative.
...
- Move the issue and pull request to `In review` only through the successful exact-head CI lifecycle handoff owned by `$labello-ready-pr-for-review`.
...
3. Continue with `$labello-ready-pr-for-review`, passing the draft result, linked issue, recorded issue status, and authorized project metadata. That skill owns hosted CI observation, PR draft state, assignment to the pull-request author, preservation of reviewer requests, and the gated `In review` transition.
...
After successful exact-head CI and the lifecycle review handoff, end with `READY FOR REVIEW`, not “complete,” “accepted,” or “verified.” Provide:
...
Recommend a fresh `$labello-verify-issue` pass as the independent review phase, but do not request any GitHub user as a reviewer. Do not close the issue, move it to Done, merge, or claim independent acceptance unless the verification verdict supports it and the user explicitly requests the relevant action.
